...
#### Your team wants to avoid duplicating the same pipeline logic in 15 Jenkinsfiles. How would you design and implement a Jenkins Shared Library for this? What would you keep in the Jenkinsfile, and what would you move into the Shared Library?
> we had several microservices with almost identical pipeline stages like checkout, build, unit testing, SonarQube analysis, Docker image creation, Trivy scanning, and deployment. Instead of maintaining the same pipeline logic in every repository, we created a Jenkins Shared Library for the common functionality.

We kept the service-specific configuration, like the application name, image name, environment, and deployment details, in the individual Jenkinsfile, while the common implementation was maintained in the Shared Library. We used reusable functions and organized the library using vars, src, and resources where required.

This gave us code reusability, consistent CI/CD standards, and easier maintenance. For example, if we needed to change the Trivy scan or add a common security step, we could update the Shared Library instead of modifying all 15 Jenkinsfiles. We also controlled the library through Git versioning so we could test and safely roll out library changes.
